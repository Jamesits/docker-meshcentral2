...
DownloadAgent() {
  ...
  then
    ...
    wget $url/meshsettings?id=$meshid -O /usr/local/mesh/meshagent.msh || curl -L --output /usr/local/mesh/meshagent.msh $url/meshsettings?id=$meshid
    ...
    then
      ...
      then
        ...
        echo 'To stop service: sudo systemctl stop meshagent'
          ...
          then
          # initd
          wget $url/meshagents?script=2 -O /etc/init.d/meshagent || curl -L --output /etc/init.d/meshagent $url/meshagents?script=2
          ...
          then
		  # FreeBSD
          wget $url/meshagents?script=5 -O /usr/local/etc/rc.d/meshagent || curl -L --output /usr/local/etc/rc.d/meshagent $url/meshagents?script=5
